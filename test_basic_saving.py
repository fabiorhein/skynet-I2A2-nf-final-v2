...
def test_basic_document_saving():
    """Test basic document saving without external dependencies."""
    print("=== Testing Basic PostgreSQL Document Saving ===\n")
    ...
    try:
        ...
        update_result = storage.save_fiscal_document({
            **retrieved_doc,
            'validation_status': 'completed',
            'document_number': '987654321'
        })
        print(f"✅ Document update result: {update_result['validation_status']}")

        # Test document deletion
        print("\n7. Testing document deletion...")
        delete_result = storage.delete_fiscal_document(saved_doc['id'])
        print(f"✅ Document deletion result: {delete_result}")

        # Verify deletion
        deleted_doc = storage.get_fiscal_document(saved_doc['id'])
...
if __name__ == "__main__":
    success = test_basic_document_saving()
