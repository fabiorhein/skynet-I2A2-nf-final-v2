...
-- Add comprehensive support for RAG (Retrieval-Augmented Generation) system

SELECT 'Starting comprehensive RAG support migration...' as migration_status;
...
-- Update fiscal_documents table with additional fields for RAG
...
-- Update document_summaries table
ALTER TABLE document_summaries 
ADD COLUMN IF NOT EXISTS embedding_vector_new VECTOR(768);

UPDATE document_summaries
SET embedding_vector_new = embedding_vector
WHERE embedding_vector IS NOT NULL;

ALTER TABLE document_summaries 
DROP COLUMN IF EXISTS embedding_vector CASCADE;
...
DROP TRIGGER IF EXISTS update_fiscal_documents_updated_at ON fiscal_documents;
...
COMMENT ON COLUMN document_chunks.embedding IS '768-dimensional vector embedding from Gemini';
COMMENT ON COLUMN document_chunks.metadata IS 'Additional metadata for filtering and context';

SELECT 'Comprehensive RAG support migration completed successfully' as migration_status;